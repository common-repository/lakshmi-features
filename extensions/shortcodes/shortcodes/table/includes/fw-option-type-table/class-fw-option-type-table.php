...
class FW_Option_Type_Table extends FW_Option_Type
{
	...
	protected function _enqueue_static( $id, $option, $data ) {
		...
		$localization_array = array(
			'msgEdit' => __( 'Edit', 'lakshmi-features' ),
	...
	protected function _render( $id, $option, $data ) {
		...
		if ( ! $table_shortcode ) {
			trigger_error(
				__( 'table-builder option type must be inside the table shortcode', 'lakshmi-features' ),
				E_USER_ERROR
			);
		}
	...
	protected function _get_defaults() {
		return apply_filters( 'fw_option_type_table_defaults', array(
			'header_options'  => array(
				'table_purpose' => array(
					'type'    => 'select',
					'label'   => __( 'Table Styling', 'lakshmi-features' ),
					'desc'    => __( 'Choose the table styling options', 'lakshmi-features' ),
					'choices' => array(
						'tabular' => __( 'Use the table to display tabular data', 'lakshmi-features' ),
					),
					'value'   => 'tabular',
					'attr'    => array(
			...
			'row_options'     => array(
				'name' => array(
					...
					'desc'    => false,
					'choices' => array(
						'default-row' => __( 'Default row', 'lakshmi-features' ),
						'heading-row' => __( 'Heading row', 'lakshmi-features' ),
						'pricing-row' => __( 'Pricing row', 'lakshmi-features' ),
						'button-row'  => __( 'Button row', 'lakshmi-features' ),
						'switch-row'  => __( 'Row switch', 'lakshmi-features' )
					),
				)
			),
			'columns_options' => array(
				'name' => array(
					...
					'desc'    => false,
					'choices' => array(
						'default-col'   => __( 'Default column', 'lakshmi-features' ),
						'desc-col'      => __( 'Description column', 'lakshmi-features' ),
						'highlight-col' => __( 'Highlight column', 'lakshmi-features' ),
						'center-col'    => __( 'Center text column', 'lakshmi-features' )
					),
				)
			),
			'content_options' => array(
				...
				'pricing-row' => array(
					...
					'description' => array(
						...
						'value'        => '',
						'attr'         => array(
							'placeholder' => __( 'per month', 'lakshmi-features' )
						),
						'wrapper_attr' => array(
							'class' => 'fw-col-sm-6'
						)
				...
				'button-row'  => array(
					'button' => ( $button = fw()->extensions->get( 'shortcodes' )->get_shortcode( 'button' ) )
						? array(
							'type'          => 'popup',
							'popup-title'   => __( 'Button', 'lakshmi-features' ),
							'button'        => __( 'Add', 'lakshmi-features' ),
